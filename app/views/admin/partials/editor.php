 ...
 *
 * @var string $name      nom del camp del formulari
 * @var string $value     HTML actual
 * @var string $id        identificador del textarea (per a l'etiqueta del camp)
 * @var string $label     nom del camp, per a qui fa servir un lector de pantalla
 * @var int    $rows      alçada aproximada, en línies
 * @var string $placeholder text que es veu mentre està buit
 */
$id = $id ?? 'ed_' . preg_replace('/[^a-z0-9_]/i', '_', $name);
$label = $label ?? 'Contingut';
$rows = max(3, (int) ($rows ?? 12));
$placeholder = $placeholder ?? 'Escriviu aquí el contingut…';
$buttons = [
...
<div class="editor" data-editor>
  <div class="editor__bar">
    <?php foreach ($buttons as [$command, $text, $hint, $style]): ?>
      <button type="button" class="editor__btn" data-command="<?= e($command) ?>"
              title="<?= e($hint) ?>" style="<?= e($style) ?>"><?= e($text) ?></button>
    <?php endforeach; ?>
  ...
  <div class="editor__area" contenteditable="true" role="textbox" aria-multiline="true"
       aria-label="<?= e($label) ?>" data-placeholder="<?= e($placeholder) ?>"
       style="min-height:<?= max(120, $rows * 24) ?>px" data-editor-area></div>
  <textarea class="editor__source" id="<?= e($id) ?>" name="<?= e($name) ?>" rows="<?= $rows ?>"
